...
class ReptilianService extends Service {
  ...
  async getData(cnodeUrl = 'https://www.csdn.net/', rule, ele1) {
    ...
    superagent.get(cnodeUrl).end(function(err, res) {
      ...
      // 设置所有的url
      $('.entry-list .item .title').each(function(idx, element) {
        var $element = $(element);
        var href = url.resolve(cnodeUrl, $element.attr('href'));
        topicUrls.push(href);
        console.log($('.entry-list .item .title').length);
      });
        ...
        topics = topics.map(function(topicPair) {
          ...
          let obj = {
            title: $('.article-title')
              .text()
              .trim(),
            author: $('.author-info-box .username')
              .text()
              .trim(),
            time: $('.meta-box .time')
              .eq(0)
              .text()
              .trim(),
            comment: $('.article-content').html()
          };

          if (obj.title != '' && obj.time != '') {
            // 爬到的数据存在就暂时放入数据库
